...
**Current results (v2, retrained on the cleaned dataset):**
- Backboard detection: precision 0.80, recall 0.87, mAP50 0.95 — recall and mAP50 both improved over v1 (0.93/0.71/0.885), despite the label fix below only touching basketball annotations. Plausible explanation: 212 corrupted labels were injecting noisy gradients into the shared backbone, degrading the whole network's feature learning, not just the basketball head.
- Basketball detection: precision 0.69, recall 0.47, mAP50 0.48 — roughly flat versus v1 (0.75/0.50/0.516) on this small 64-image validation split; still misses close to half of real balls. Aggregate detection quality on this class remains a real, open problem — likely needs more labeled frames, not another dataset-hygiene fix.
- **Root cause found and fixed for the recurring false positive**: a script scanning the raw YOLO label files directly found that 212 of 640 training images (33%) had a `basketball` label covering ~99.6% of the frame — almost certainly an unreviewed Roboflow SAM3 auto-labeler fallback artifact, not a real annotation. Two earlier theories were tested and ruled out first: a fixed background object (ruled out once the same false positive appeared in a completely different venue) and a 640×640 letterbox-padding artifact (an A/B test comparing letterboxed vs. pre-stretched inference was inconclusive, since the initial sampled frames didn't contain the false positive at all — a wider scan was needed to relocate it before the label inspection pointed at the real cause). After stripping the contaminated labels and retraining, a targeted recheck against the exact three frames where the false positive originally appeared confirmed it is gone at every confidence level tested, including well below the original 0.7 mitigation threshold.
...
**Not yet built:** improving basketball detection recall, resolving an observed backboard hoop-side inconsistency (a promising angle: the backboard and rim are physically attached with the backboard being the stronger-performing class, so it may work well as a geometric anchor to constrain rim search rather than treating rim detection as fully independent), the actual rim-crossing/trajectory math, and Django integration (deliberately last, once the CV pipeline works standalone).
